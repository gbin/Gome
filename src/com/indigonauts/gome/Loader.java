...
public class Loader extends Canvas implements Runnable, CommandListener {
    ...
    protected void paint(Graphics g) {
        ...
        g.drawString("(c) 2005-2007 Indigonauts", getWidth() / 2, getHeight() / 2 + f.getHeight(), Graphics.BASELINE
                                                                                                   | Graphics.HCENTER);
        ...
        try {
            ...
            if (Gome.singleton.options.user.length() == 0
                ...
                if (Gome.singleton.options.expiration != 0
                    && System.currentTimeMillis() > Gome.singleton.options.expiration) {
                    Alert al = new Alert(Gome.singleton.bundle.getString("ui.expired"), Gome.singleton.bundle
                            .getString("ui.expiredExplanation"), null, AlertType.ERROR);
    ...
    private void bootGome() {
        ...
        if (Gome.singleton.options.user.length() != 0) {
            ...
            message = Gome.singleton.bundle.getString("ui.daysLeft", new String[] { String
                    .valueOf((Gome.singleton.options.expiration -System.currentTimeMillis()) / (24 * 60 * 60 * 1000L)) });
        }
