...
public class MenuEngine implements CommandListener {
    ...
    static {
        ...
        EXIT = new Command(Gome.singleton.bundle.getString("ui.exit"), Command.EXIT, 10); //$NON-NLS-1$
        ...
        CHANGE_ONLINE_HANDICAP = new Command(
                                             Gome.singleton.bundle.getString("online.changeHandicap"), Command.SCREEN, 5); //$NON-NLS-1$

        IGS_RESET_DEADS_TONE = new Command(Gome.singleton.bundle.getString("count.undoMarkDeadStone"), Command.SCREEN,
                                           5);
        ...
        try {
            ...
            if (d == Gome.singleton.mainCanvas) {
                if (c == NEW) {
                    ...
                    try {
                        fileBrowser = new FileBrowser(Gome.singleton.mainCanvas, this, GamesIOManager
                                .getAllLocallyAccessibleGamesList(), false);
                        fileBrowser.show(Gome.singleton.display);
                    } catch (IOException e1) {
                        Util
                                .messageBox(
                                            Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString(e1.getMessage()), AlertType.ERROR); //$NON-NLS-1$
                    }
                } else if (c == LIBRARY) {
                    try {
                        fileBrowser = new FileBrowser(Gome.singleton.mainCanvas, this, GamesIOManager
                                .getAllLocallyAccessibleGamesList(), true);
                        fileBrowser.show(Gome.singleton.display);
                    } catch (IOException e1) {
                        Util
                                .messageBox(
                ...
                if (c == SAVE) {
                    try {
                        ...
                        Util
                                .messageBox(
                                            Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString(e.getMessage()), AlertType.ERROR); //$NON-NLS-1$
                    }
                }
                saveGameForm = null;
                Gome.singleton.mainCanvas.show(Gome.singleton.display);
    ...
    public void deleteFile(FileEntry file) {
        if (file instanceof StoreFileEntry) {
            try {
                ...
                Util
                        .messageBox(
                                    Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString("ui.error.delete"), AlertType.ERROR); //$NON-NLS-1$ //$NON-NLS-2$
            }
        } else {
            Util
                    .messageBox(
                                Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString("ui.error.wrongType"), AlertType.ERROR); //$NON-NLS-1$ //$NON-NLS-2$
        }
    }

    public void showIgsChallenge(ServerChallenge challenge) {
        currentChallenge = challenge;
        String colors = ((challenge.color == Board.BLACK) ? Gome.singleton.bundle.getString("game.blackLong")
                                                         : Gome.singleton.bundle.getString("game.whiteLong"));
        challengeForm = new Form(challenge.nick + Gome.singleton.bundle.getString("online.challengesYou"));
        String byoORnot = ((challenge.min_per25moves != -1) ? " + " + challenge.min_per25moves
                                                              + Gome.singleton.bundle.getString("clock.min25stones")
                                                           : Gome.singleton.bundle.getString("online.noByo"));
        String args[] = { colors, String.valueOf(challenge.size), String.valueOf(challenge.size),
                         String.valueOf(challenge.time_minutes), byoORnot };
