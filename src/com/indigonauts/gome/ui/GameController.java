...
public class GameController implements FileLoaderCallback, ServerCallback {
    ...
    public void notifyLoadReady() {
        ...
        if (playArea == null || !playArea.isValid()) {
            playArea = new Rectangle((byte) 0, (byte) 0, (byte) (board.getBoardSize() - 1), (byte) (board
                    .getBoardSize() - 1));
        }
    ...
    private void warnPassed(byte color) {
        if (!countMode)
            canvas.setSplashInfo((color == 1 ? Gome.singleton.bundle.getString("game.blackLong")
                    : Gome.singleton.bundle.getString("game.whiteLong"))
                    + Gome.singleton.bundle.getString("game.passed"));
    ...
    private void switchCurrentNode(SgfNode node) {
        ...
        if ((playMode == PROBLEM_MODE) && currentNode.getFather() != null
                && currentNode.getFather().getComment() != null) {
    ...
    public void newGame(byte size, int handi, char gameMode) {
        gameHasEnded = false;
        playMode = gameMode;
        SgfModel game = SgfModel.createNewModel(size, handi,
                Gome.singleton.bundle.getString("game.whiteLong"), Gome.singleton.bundle.getString("game.blackLong")); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void gameIsDone(String name1, int value1, String name2, int value2) {
        ...
        if (value1 > value2) {
            ...
            out = name1 + " (" + v1 + ") " + name2 + " (" + v2 + ") " + name1
                    + Gome.singleton.bundle.getString("game.winBy") + v3;
        }
        if (value1 < value2) {
            String v3 = String.valueOf(value2 - value1);
            v3 = v3.substring(0, v3.length() - 1) + "." + v3.substring(v3.length() - 1);

            out = name2 + " (" + v2 + ") " + name1 + " (" + v1 + ") " + name2
                    + Gome.singleton.bundle.getString("game.winBy") + v3;
        }
    ...
    public void onlineScore(int whiteScore, int blackScore) {
        ...
        whiteScoreString = whiteScoreString.substring(0, whiteScoreString.length() - 1) + "."
                + whiteScoreString.substring(whiteScoreString.length() - 1);

        String blackScoreString = String.valueOf(blackScore);
        blackScoreString = blackScoreString.substring(0, blackScoreString.length() - 1) + "."
                + blackScoreString.substring(blackScoreString.length() - 1);

        canvas.setSplashInfo(Gome.singleton.bundle.getString("game.score") + black + ":" + blackScoreString + " "
                + white + ": " + whiteScoreString);
    }
