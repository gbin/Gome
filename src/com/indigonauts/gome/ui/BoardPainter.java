...
public class BoardPainter {
	...
	private static final Font ANNOTATION_FONT_SMALL = Font.getFont(
			Font.FACE_PROPORTIONAL, Font.STYLE_BOLD, Font.SIZE_SMALL);

	private static final Font ANNOTATION_FONT_MEDIUM = Font.getFont(
			Font.FACE_PROPORTIONAL, Font.STYLE_BOLD, Font.SIZE_MEDIUM);

	private static final Font ANNOTATION_FONT_LARGE = Font.getFont(
	...
	public void drawBoard(Graphics graphics) {
		...
		graphics.setClip(ux - halfdelta, uy - halfdelta,
				getCellX(boardArea.x1 + 1) - ux + 1, getCellY(boardArea.y1 + 1)
						- uy + 1);
		graphics.setColor(Util.COLOR_WHITE);
		graphics.drawRect(ox - halfdelta, oy - halfdelta, oxx - ox + halfdelta
				+ halfdelta, oyy - oy + halfdelta + halfdelta);
		...
		while (e.hasMoreElements()) {
			...
			if (getBoardArea().contains(annotation)) {
				...
				if (annotation instanceof TextAnnotation) {
					...
					drawTextAnnotation(g, annotation, textAnnotation.getText(),
							position == 0, color);
				} else // so it is a symbol
				{
					drawSymbolAnnotation(g, (SymbolAnnotation) annotation,
							color);
	...
	public void drawTerritory(Graphics g) {
		...
		int whiteTerritoryColor = Util.blendColors(
				Gome.singleton.options.gobanColor, Util.COLOR_WHITE);
		int blackTerritoryColor = Util.blendColors(
				Gome.singleton.options.gobanColor, Util.COLOR_BLACK);
		int blackTerritoryColorOnWhiteStone = Util.blendColors(
	...
	private void calcDrawingPosition() {
		int deltax = (drawArea.getWidth() - (MARGIN * 2))
				/ boardArea.getWidth();
		int deltay = (drawArea.getHeight() - (MARGIN * 2))
				/ boardArea.getHeight();
	...
	public void drawTextAnnotation(Graphics g, Point pt, String text,
			boolean erasebg, int color) {
		...
		g.drawString(text, cx, cy - offset + halfdelta, Graphics.BOTTOM
				| Graphics.HCENTER);
	}

	public void drawSymbolAnnotation(Graphics g, SymbolAnnotation annotation,
			int color) {
