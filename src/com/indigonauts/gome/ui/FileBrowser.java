...
public class FileBrowser implements CommandListener, Showable,
	...
	public static Command SELECT_ALL;
	...
	private static final GraphicRectangle ILLUSTRATIVE_RECTANGLE = new GraphicRectangle(
			1, 1, ILLUSTRATIVE_SIZE - 2, ILLUSTRATIVE_SIZE - 2);
	...
	public FileBrowser(Showable parent, MenuEngine listener,
		...
		OPEN = new Command(
				Gome.singleton.bundle.getString("ui.open"), Command.SCREEN, 2); //$NON-NLS-1$

		DELETE = new Command(
				Gome.singleton.bundle.getString("ui.delete"), Command.SCREEN, 3); //$NON-NLS-1$

		SELECT_ALL = new Command(Gome.singleton.bundle
				.getString("ui.selectall"), Command.SCREEN, 3); //$NON-NLS-1$

		IMPORT = new Command(
				Gome.singleton.bundle.getString("ui.import"), Command.SCREEN, 2); //$NON-NLS-1$

        SEND_BY_EMAIL = new Command(Gome.singleton.bundle
				.getString("ui.sendByEmail"), Command.SCREEN, 2); //$NON-NLS-1$

		RANDOM = new Command(
				Gome.singleton.bundle.getString("ui.random"), Command.SCREEN, 2); //$NON-NLS-1$
	...
	public void show(Display disp) {
		uiFolder = new List(
				Gome.singleton.bundle.getString("ui.fileselect"), managementMode ? Choice.MULTIPLE //$NON-NLS-1$
						: Choice.IMPLICIT);
		...
		while (all.hasMoreElements()) {
			...
			if (current instanceof CollectionEntry) {
				...
				if (file.hasAnIllustration()) {
					image = generateIllustrativePosition(file
							.getIllustrativeBoardArea(), file
							.getIllustrativeBlackPosition(), file
							.getIllustrativeWhitePosition());
				} else
					image = file.isRemote() ? REMOTE_FILE : FILE;
				uiFolder.append(description
						+ (fileNum != 1 ? " [" + fileNum + "]" : ""), //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						image);
		...
		if (managementMode) {
			...
			uiFolder.addCommand(SELECT_ALL);
	...
	private Image generateIllustrativePosition(String boardArea, String black,
			String white) {
		StringVector blackPoints = new StringVector(black, ';');
		StringVector whitePoints = new StringVector(white, ';');
		Image generated = Image.createImage(ILLUSTRATIVE_SIZE,
				ILLUSTRATIVE_SIZE);
		Board position = new Board();
		Enumeration bpoints = blackPoints.elements();
		while (bpoints.hasMoreElements()) {
			position.placeStone(SgfPoint.createFromSgf((String) bpoints
					.nextElement()), Board.BLACK);
		}
		Enumeration wpoints = whitePoints.elements();
		while (wpoints.hasMoreElements()) {
			position.placeStone(SgfPoint.createFromSgf((String) wpoints
					.nextElement()), Board.WHITE);
		}

		StringVector boardAreaSplitted = new StringVector(boardArea, ';');
		BoardPainter bp = new BoardPainter(
				position,
				ILLUSTRATIVE_RECTANGLE,
				new Rectangle(SgfPoint.createFromSgf((String) boardAreaSplitted
						.elementAt(0)), SgfPoint
		...
		if (s == uiFolder) {
			...
			if (c == OPEN || (!managementMode && c == List.SELECT_COMMAND)) {
				...
				if (entry instanceof IndexEntry) {
					...
					new IndexLoader(index.getPath(), this)
							.show(Gome.singleton.display);
					return;

				} else if (entry instanceof CollectionEntry
						&& ((CollectionEntry) entry).getCollectionSize() == 1) {
					indexBlock = 0;
					selectedNum = 0;
				...
				boolean[] all = new boolean[uiFolder.size()];
				uiFolder.getSelectedFlags(all);
				Vector allToDelete = new Vector();
				for (int i = 0; i < all.length; i++) {
					if (all[i]) {
						Object obj = entries.elementAt(i);
						if (obj instanceof StoreFileEntry) {
							StoreFileEntry entry = (StoreFileEntry) obj;
							allToDelete.addElement(entry);
							listener.deleteFile(entry);
						}
					}

				}
				for (Enumeration iter = allToDelete.elements(); iter
						.hasMoreElements();) {
					entries.removeElement(iter.nextElement());
				}
				show(Gome.singleton.display);
			} else if (c == SELECT_ALL) {
				int size = uiFolder.size();
				boolean[] all = new boolean[size];
				for (int i = 0; i < size; i++) {
					all[i] = true;
				}
				uiFolder.setSelectedFlags(all);
				...
				FileEntry selectedFile = getSelectedFile();
				IOManager.sendFileByMail(selectedFile, Gome.singleton.options.email);
	...
	private void showFileBlock() {
		...
		int all = ((CollectionEntry) entries.elementAt(indexFolder))
				.getCollectionSize();
		int remain = all;
		int n = 0;
	...
	public void importFile(FileEntry selectedFile) {
		boolean typeok = true;
		if (selectedFile instanceof StoreFileEntry
				|| selectedFile instanceof BundledFileEntry) {
			typeok = false;
		...
		if (!typeok) {
			Util
					.messageBox(
							"ui.error", Gome.singleton.bundle.getString("ui.error.onlyOnline"), AlertType.ERROR); //$NON-NLS-1$ //$NON-NLS-2$
			return;
		}

		FileImporter fileLoader = new FileImporter(new ImportCallback(this),
				selectedFile);
	...
	public void downloadFailure(Exception reason) {
		Util
				.messageBox(
						Gome.singleton.bundle.getString("ui.failure"), reason.getMessage(), AlertType.ERROR); //$NON-NLS-1$
	}
}