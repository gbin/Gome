...
public class IOManager {
    ...
    public DataInputStream readFileFromHttp(String url, DownloadStatus status) throws IOException {
        ...
        while (true) {
            ...
            if (currentHttpConnection.getResponseCode() == 401
                && currentHttpConnection.getHeaderField("WWW-Authenticate").startsWith("Basic")) {
            ...
            public void run() {
                ...
                try {
                    HttpConnection httpConnection = (HttpConnection) Connector
                            .open(urlWithParams, Connector.READ_WRITE);
                    ...
                    Util
                            .messageBox(
                                        Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString("ui.error.recordStored"), AlertType.ERROR); //$NON-NLS-1$ //$NON-NLS-2$
                    return;

                } catch (IOException e) {
                    Util
                            .messageBox(
                                        Gome.singleton.bundle.getString("ui.error"), Gome.singleton.bundle.getString("ui.error.posting"), AlertType.ERROR); //$NON-NLS-1$ //$NON-NLS-2$
                    return;
                ...
                Util
                        .messageBox(
                                    Gome.singleton.bundle.getString("ui.success"), Gome.singleton.bundle.getString("ui.email.success"), AlertType.INFO); //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public static String URLEncode(String s) {
        ...
        for (int i = 0; i < length; i++) {
            char b = s.charAt(i);
            if ((b >= (char) 0x30 && b <= (char) 0x39) || (b >= (char) 0x41 && b <= (char) 0x5A)
                || (b >= (char) 0x61 && b <= (char) 0x7A)) {
                tmp.append(b);
            } else {
                tmp.append('%');
                if (b <= (char) 0xf)
