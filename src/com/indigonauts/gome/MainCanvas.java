...
public class MainCanvas extends Canvas implements CommandListener, Showable, Runnable {
    ...
    private void assignUnassignedKey() {
        ...
        int[] allActions = { Canvas.GAME_A, Canvas.GAME_B, Canvas.GAME_C, Canvas.GAME_D, Canvas.UP, Canvas.DOWN,
                            Canvas.LEFT, Canvas.RIGHT, Canvas.FIRE };
    ...
    protected void keyPressed(int keyCode) {
        ...
        boolean review = playMode == GameController.REVIEW_MODE || playMode == GameController.JOSEKI_MODE
                         || playMode == GameController.OBSERVE_MODE;
    ...
    public void paint(Graphics g) {
        ...
        if (scroller.getX() == g.getClipX() && scroller.getY() == g.getClipY()
            && scroller.getWidth() == g.getClipWidth() && scroller.getHeight() == g.getClipHeight()) {
            //log.debug("Refresh only the bottom");
            drawStatusBar(g);
        } else if (bottomMode == CLOCK_MODE && clockPainter.getX() == g.getClipX()
                   && clockPainter.getY() == g.getClipY() && clockPainter.getWidth() == g.getClipWidth()
    ...
    private void startScroller() {
        ...
        img = Util.renderOffScreenScrollableText(currentComment != null ? currentComment : "#" + gc.getMoveNb(),
                                                 getWidth(), scroller.getHeight(), scrollerFont, Util.COLOR_BLACK, //$NON-NLS-1$ //$NON-NLS-2$
    ...
    public void displayHelp() {
        ...
        char playMode = gc.getPlayMode();
        if (playMode == GameController.JOSEKI_MODE || playMode == GameController.OBSERVE_MODE
            || playMode == GameController.REVIEW_MODE) {
